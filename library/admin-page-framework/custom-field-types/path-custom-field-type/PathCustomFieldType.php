...
if ( ! class_exists( 'FacultyWeeklySchedule_PathCustomFieldType' ) ) :
include( dirname( __FILE__ ) . '/FacultyWeeklySchedule_PathCustomFieldType_Node.php' );
/**
 ...
 * @version     0.0.1
 */
class FacultyWeeklySchedule_PathCustomFieldType extends FacultyWeeklySchedule_AdminPageFramework_FieldType_image {
    ...
    protected function construct() {
        ...
        add_action( 'wp_ajax_apf_path_field_type-faculty-weekly-schedule' , '__return_empty_string' );
    }
        ...
        return array(
            ...
            array(
                ...
                'translation'    => array(
                    ...
                    'nonce'     => $_sNonce,
                    'label'     => array(
                        'selectPath' => __( 'Select Path', 'faculty-weekly-schedule' ),
                        'select'     => __( 'Select', 'faculty-weekly-schedule' ),
                    ),
                ),
                'translation_var' => 'FacultyWeeklySchedule_AdminPageFrameworkPathFieldType',
            ),
        );
    }
    ...
    public function getField( $aField ) {
        ...
        return $aField[ 'before_label' ]
            . "<div class='faculty-weekly-schedule-input-label-container faculty-weekly-schedule-input-container {$aField[ 'type' ]}-field'>" 
                . "<label for='{$aField[ 'input_id' ]}'>"
                    . $aField[ 'before_input' ]
                    . $this->getAOrB(
                        $aField[ 'label' ] && ! $aField[ 'repeatable' ],
                        "<span " . $this->getLabelContainerAttributes( $aField, 'faculty-weekly-schedule-input-label-string' ) . ">" 
                            . $aField[ 'label' ] 
                        . "</span>",                        
                        ''                        
                    )
         ...
         * @since       3.8.4
         * @since       3.9.0       Moved from `FacultyWeeklySchedule_PathCustomFieldType`.
         * @param       array       $aField
            ...
            return "<a " . $this->getAttributes( $_aAttributes ) . ">"
                    . $this->getAOrB(
                        ...
                        $this->getAOrB(
                            strrpos( $_aAttributes[ 'class' ], 'dashicons' ),
                            '',
                            __( 'Select Path', 'faculty-weekly-schedule' )
                        )
                    )
                ."</a>";
    ...
    protected function doOnFieldRegistration( $aFieldset ) {
        ...
        private function ___handleQuery( $aFieldset ) {
            ...
            if ( empty( $_POST[ 'faculty-weekly-schedule_path_field_type' ] ) ||  empty( $_POST[ 'nonce' ] ) ) {
                return;
            }
            ...
            $_sRelativePath = empty( $_POST[ 'id' ] ) || '#' === $_sNodeID
                ? '/'
                : trim( $_sNodeID, '\\/' );
            $_oTreeNode     = new FacultyWeeklySchedule_PathCustomFieldType_Node( $_sRelativePath, $_sRootDirPath, $_aPathOptions );
