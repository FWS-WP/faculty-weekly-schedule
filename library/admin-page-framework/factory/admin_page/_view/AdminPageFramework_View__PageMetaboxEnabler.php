...
class FacultyWeeklySchedule_AdminPageFramework_View__PageMetaboxEnabler extends FacultyWeeklySchedule_AdminPageFramework_FrameworkUtility {
    public $oFactory;
    ...
    {
        $_aPageMetaBoxClasses = $this->getElementAsArray($GLOBALS, array( 'aFacultyWeeklySchedule_AdminPageFramework', 'aMetaBoxForPagesClasses' ));
    ...
    {
        $_bLoaded = $this->getElement($GLOBALS, array( 'aFacultyWeeklySchedule_AdminPageFramework', 'bAddedMetaBoxScript' ), false);
        if ($_bLoaded) {
            return;
        }
        $GLOBALS[ 'aFacultyWeeklySchedule_AdminPageFramework' ][ 'bAddedMetaBoxScript' ] = true;
        $_sScript = <<<JAVASCRIPTS
jQuery(document).ready(function(){postboxes.add_postbox_toggles(pagenow)})
JAVASCRIPTS;
        echo '<script class="faculty-weekly-schedule-insert-metabox-script">' . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . '</script>';
    }
}
