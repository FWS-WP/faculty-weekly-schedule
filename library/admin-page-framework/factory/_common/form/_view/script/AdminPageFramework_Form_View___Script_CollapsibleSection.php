...
class FacultyWeeklySchedule_AdminPageFramework_Form_View___Script_CollapsibleSection extends FacultyWeeklySchedule_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeFacultyWeeklySchedule_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'faculty-weekly-schedule_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeFacultyWeeklySchedule_AdminPageFrameworkCollapsibleSections();
    });    
    ...
    $.fn.initializeFacultyWeeklySchedule_AdminPageFrameworkCollapsibleSections = function() {
        ...
        jQuery( this ).find( '.faculty-weekly-schedule-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.faculty-weekly-schedule-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.faculty-weekly-schedule-collapsible-section-title[data-is_collapsed=\"0\"]' )
        ...
        jQuery( this ).find( '.faculty-weekly-schedule-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.faculty-weekly-schedule-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.faculty-weekly-schedule-collapsible-sections-title, .faculty-weekly-schedule-collapsible-section-title' )
            .enableFacultyWeeklySchedule_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.faculty-weekly-schedule-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'faculty-weekly-schedule-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.faculty-weekly-schedule-sectionset' ).siblings().andSelf().find( '> .faculty-weekly-schedule-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.faculty-weekly-schedule-collapsible-toggle-all-button-container' ).andSelf();
    ...
    $.fn.enableFacultyWeeklySchedule_AdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'faculty-weekly-schedule-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'faculty-weekly-schedule-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.faculty-weekly-schedule-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .faculty-weekly-schedule-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.faculty-weekly-schedule-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='faculty-weekly-schedule-collapsible-toggle-all-button-container'>" . "<span class='faculty-weekly-schedule-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
    }
