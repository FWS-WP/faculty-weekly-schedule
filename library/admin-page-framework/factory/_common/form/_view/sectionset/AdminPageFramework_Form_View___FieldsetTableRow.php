...
class FacultyWeeklySchedule_AdminPageFramework_Form_View___FieldsetTableRow extends FacultyWeeklySchedule_AdminPageFramework_Form_View___Section_Base {
    ...
    public function get()
    {
        ...
        $_oFieldrowAttribute = new FacultyWeeklySchedule_AdminPageFramework_Form_View___Attribute_Fieldrow($aFieldset, array( 'id' => 'fieldrow-' . $aFieldset[ 'tag_id' ], 'valign' => 'top', 'class' => 'faculty-weekly-schedule-fieldrow', ));
        return $this->_getFieldByContainer($aFieldset, array( 'open_container' => "<tr " . $_oFieldrowAttribute->get() . ">", 'close_container' => "</tr>", 'open_title' => "<th>", 'close_title' => "</th>", 'open_main' => "<td " . $this->getAttributes(array( 'colspan' => $aFieldset[ 'show_title_column' ] ? 1 : 2, 'class' => $aFieldset[ 'show_title_column' ] ? null : 'faculty-weekly-schedule-field-td-no-title', )) . ">", 'close_main' => "</td>", ));
    }
    protected function _getFieldByContainer(array $aFieldset, array $aOpenCloseTags)
    {
        ...
        $_oFieldTitle = new FacultyWeeklySchedule_AdminPageFramework_Form_View___FieldTitle($aFieldset, '', $this->aSavedData, $this->aFieldErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
        $_aOutput = array();
