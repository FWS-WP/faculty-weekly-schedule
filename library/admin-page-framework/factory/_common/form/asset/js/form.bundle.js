...
(function ( $ ) {
     ...
     */
    $.fn.selectAllFacultyWeeklySchedule_AdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
     ...
     */
    $.fn.deselectAllFacultyWeeklySchedule_AdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
            .find( 'input[type=checkbox]' )
            .prop( 'checked', false )
            .trigger( 'change' );   // 3.8.8+
...
( function( $ ) {
    ...
    var _removeFacultyWeeklySchedule_AdminPageFrameworkLoadingOutputs = function() {

        jQuery( '.faculty-weekly-schedule-form-loading' ).remove();
        jQuery( '.faculty-weekly-schedule-form-js-on' )
            .hide()
            .css( 'visibility', 'visible' )
            .fadeIn( 200 )
            .removeClass( '.faculty-weekly-schedule-form-js-on' );
    ...
    window.onerror = function(){
        ...
        _removeFacultyWeeklySchedule_AdminPageFrameworkLoadingOutputs();
...
(function ( $ ) {
    ...
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkInputOptions = {};

    $.fn.storeFacultyWeeklySchedule_AdminPageFrameworkInputOptions = function( sID, vOptions ) {
        ...
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkInputOptions[ sID ] = vOptions;
    };
    $.fn.getFacultyWeeklySchedule_AdminPageFrameworkInputOptions = function( sID ) {
        sID = sID.replace( /__\d+_/, '___' ); // remove the section index
        return ( 'undefined' === typeof $.fn.aFacultyWeeklySchedule_AdminPageFrameworkInputOptions[ sID ] )
            ? null
            : $.fn.aFacultyWeeklySchedule_AdminPageFrameworkInputOptions[ sID ];
    }
...
(function ( $ ) {
    ...
    // Callback containers.
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkAddRepeatableFieldCallbacks        = [];
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRemoveRepeatableFieldCallbacks     = [];
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkSortedFieldsCallbacks              = [];
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkStoppedSortingFieldsCallbacks      = [];
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkAddedWidgetCallbacks               = [];
    $.fn.aFacultyWeeklySchedule_AdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+

    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */
    $( document ).on( 'faculty-weekly-schedule_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aFacultyWeeklySchedule_AdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registerFacultyWeeklySchedule_AdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     *
     * Unlike the `faculty-weekly-schedule_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).on( 'faculty-weekly-schedule_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).on( 'faculty-weekly-schedule_stopped_sorting_sections', function( oEvent ){

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aFacultyWeeklySchedule_AdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).on( 'faculty-weekly-schedule_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRemoveRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $.fn.callBackSortedFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkSortedFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
    ...
    $.fn.callBackStoppedSortingFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkStoppedSortingFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
     ...
     */
    $( document ).on( 'faculty-weekly-schedule_saved_widget', function( event, oWidget ){
        $.each( $.fn.aFacultyWeeklySchedule_AdminPageFrameworkAddedWidgetCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 'function' !== typeof _hfCallback ) {
                return true;    // continue
     ...
     */
    $.fn.registerFacultyWeeklySchedule_AdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkAddRepeatableFieldCallbacks.push(
            [ oCallbacks.added_repeatable_field, aFieldTypeSlugs ]
        );

        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
            [ oCallbacks.repeated_field, aFieldTypeSlugs ]
        );
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRemoveRepeatableFieldCallbacks.push(
            [ oCallbacks.removed_repeatable_field, aFieldTypeSlugs ]
        );
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkSortedFieldsCallbacks.push(
            [ oCallbacks.sorted_fields, aFieldTypeSlugs ]
        );
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkStoppedSortingFieldsCallbacks.push(
            [ oCallbacks.stopped_sorting_fields, aFieldTypeSlugs ]
        );
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkAddedWidgetCallbacks.push(
            [ oCallbacks.saved_widget, aFieldTypeSlugs ]
        );

        // 3.8.0
        $.fn.aFacultyWeeklySchedule_AdminPageFrameworkStoppedSortingSectionsCallbacks.push(
            [ oCallbacks.stopped_sorting_sections, aFieldTypeSlugs ]
        );

    };
    /**
     * An alias of the `registerFacultyWeeklySchedule_AdminPageFrameworkCalbacks()` method.
    ...
    $.fn.registerAPFCallback = function( oCallbacks, aFieldTypeSlugs ) {
        $.fn.registerFacultyWeeklySchedule_AdminPageFrameworkCallbacks( oCallbacks, aFieldTypeSlugs );
    }
...
(function ( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.faculty-weekly-schedule-fields.repeatable' ).each( function() {
            var _buttonModel = $( this ).siblings( '.repeatable-field-buttons-model' );
            if ( ! _buttonModel.length ) {
                return true;
            }
            var _aSettings           = _buttonModel.data();
            var _buttonContainer     = _buttonModel.children( '.faculty-weekly-schedule-repeatable-field-buttons' ).first();
            ...
            var _childFields = $( this ).find( '> .faculty-weekly-schedule-field.without-child-fields' );
            ...
            /* Otherwise, insert the button element at the beginning of the field tag */
                 ...
                 */
                if ( ! _childFields.find( '.faculty-weekly-schedule-repeatable-field-buttons' ).length ) {
             ...
             */
            $( this ).find( '> .faculty-weekly-schedule-field.with-child-fields' ).prepend( _buttonsNestedFields );
                /**
                 * Add buttons to the fields tag.
                 */
                // $( this ).find( '.faculty-weekly-schedule-field.with-nested-fields' ).prepend( _buttonsNestedFields );

                /**
                 * Support for inline mixed fields.
                 * @todo not sure why this is commented out. Remove this if it's okay.
                 */
                // $( this ).find( '.faculty-weekly-schedule-field.with-mixed-fields' ).prepend( _buttonsNestedFields );




            $( this ).updateFacultyWeeklySchedule_AdminPageFrameworkRepeatableFields( _aSettings ); // Update the fields
     ...
     * @var FacultyWeeklySchedule_AdminPageFrameworkScriptFormMain
     */
    var translation = FacultyWeeklySchedule_AdminPageFrameworkScriptFormMain;
     ...
     */
    $.fn.updateFacultyWeeklySchedule_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.faculty-weekly-schedule-repeatable-field-buttons' )
            .filter( function() {
                return $( this ).closest( '.faculty-weekly-schedule-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            });
        ...
        _oRepeatableAddButtons.on( 'click', function() {
            ...
            $( this ).addFacultyWeeklySchedule_AdminPageFrameworkRepeatableField();
        ...
        _oRepeatableRemoveButton.on( 'click', function() {
            ...
            $( this ).removeFacultyWeeklySchedule_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.faculty-weekly-schedule-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.faculty-weekly-schedule-field' )
            .filter( function() {
                return $( this ).closest( '.faculty-weekly-schedule-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            })
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {
                $( '#' + _sFieldID ).addFacultyWeeklySchedule_AdminPageFrameworkRepeatableField( _sFieldID );
            }
        }
     ...
     */
    $.fn.addFacultyWeeklySchedule_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {

        if ( 'undefined' === typeof sFieldContainerID ) {
            sFieldContainerID = $( this ).closest( '.faculty-weekly-schedule-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.faculty-weekly-schedule-fields' );
        ...
        if ( ! _aOptions ) {
            var _nodeButtonContainer = nodeFieldContainer.find( '.faculty-weekly-schedule-repeatable-field-buttons' )
                .filter( function() {
                    return $( this ).closest( '.faculty-weekly-schedule-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.faculty-weekly-schedule-field' )
                                    .filter( function() {
                                        return $( this ).closest( '.faculty-weekly-schedule-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.faculty-weekly-schedule-repeatable-field-buttons' )
                                    .filter( function() {
                                        return $( this ).closest( '.faculty-weekly-schedule-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
         ...
         */
        nodeNewField.updateFacultyWeeklySchedule_AdminPageFrameworkRepeatableFields();
        ...
        nodeNewField.trigger(
            'faculty-weekly-schedule_added_repeatable_field',
            [
        ...
        $( nodeNewField ).find( '.faculty-weekly-schedule-field' ).addBack().trigger(
            'faculty-weekly-schedule_repeated_field',
            [
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.faculty-weekly-schedule-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {
                return $( this ).closest( '.faculty-weekly-schedule-fields' ).attr( 'id' ) === _sFieldsContainerID;
            })
        ...
        nodeFieldsContainer.children( '.faculty-weekly-schedule-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.faculty-weekly-schedule-field,.faculty-weekly-schedule-fields,.faculty-weekly-schedule-fieldset' ).incrementAttributes(
                [ 'id', 'data-field_tag_id_model', 'data-field_id' ],
                _iFieldCount,
                _sFieldTagIDModel
            );
            oElement.find( '.faculty-weekly-schedule-fields' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeFacultyWeeklySchedule_AdminPageFrameworkRepeatableField = function() {

        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.faculty-weekly-schedule-field' );
        var nodeFieldsContainer = $( this ).closest( '.faculty-weekly-schedule-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
...
( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.faculty-weekly-schedule-sections.repeatable-section' ).each( function( index ){

            var _buttonModel    = $( this ).children( '.repeatable-section-buttons-model' ).first();
            var _repeatOptions  = _buttonModel.data();
            _buttonModel = _buttonModel.find( '.faculty-weekly-schedule-repeatable-section-buttons' ).first().detach();

            // Add buttons
            $( this ).find( '.faculty-weekly-schedule-section-caption' ).each( function(){

                $( this ).show();
                var _oButtons = $( _buttonModel[0].outerHTML );
                if ( $( this ).children( '.faculty-weekly-schedule-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
                    _oButtons.addClass( 'section_title_field_sibling' );
                }
                var _oCollapsibleSectionTitle = $( this ).find( '.faculty-weekly-schedule-collapsible-section-title' );
            ...
            // Update the fields
            $( this ).updateFacultyWeeklySchedule_AdminPageFrameworkRepeatableSections( _repeatOptions );
     ...
     */
    $.fn.updateFacultyWeeklySchedule_AdminPageFrameworkRepeatableSections = function( aSettings ) {

        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.faculty-weekly-schedule-sections' ).attr( 'id' );

        // Store the sections specific options in an array.
        if ( ! $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {
        ...
        var _aOptions = $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).on( 'click', function() {
            ...
            $( this ).addFacultyWeeklySchedule_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // The Remove button behavior
        $( _oThis ).find( '.repeatable-section-remove-button' ).on( 'click', function() {
            $( this ).removeFacultyWeeklySchedule_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // If the number of sections is less than the set minimum value, add sections.
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.faculty-weekly-schedule-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.faculty-weekly-schedule-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {
                $( '#' + _sSectionID ).addFacultyWeeklySchedule_AdminPageFrameworkRepeatableSection( _sSectionID );
            }
        }
     ...
     */
    $.fn.addFacultyWeeklySchedule_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {

        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            sSectionContainerID = $( this ).closest( '.faculty-weekly-schedule-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.faculty-weekly-schedule-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.faculty-weekly-schedule-section-tabs-contents' )
            .children( '.faculty-weekly-schedule-section-tabs' )
            .first();

        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );

        var _iFadein                = $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        var _bPreserveValues        = $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'preserve_values' ];

        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.faculty-weekly-schedule-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.faculty-weekly-schedule-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.faculty-weekly-schedule-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.faculty-weekly-schedule-section-title' ).not( '.faculty-weekly-schedule-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableFacultyWeeklySchedule_AdminPageFrameworkCollapsibleButton ){
            nodeNewSection.find( '.faculty-weekly-schedule-collapsible-sections-title, .faculty-weekly-schedule-collapsible-section-title' ).enableFacultyWeeklySchedule_AdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.faculty-weekly-schedule-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updateFacultyWeeklySchedule_AdminPageFrameworkRepeatableFields();
            ...
            $( this ).trigger(
                ...
                [
                    ...
                    jQuery( nodeNewSection ).closest( '.faculty-weekly-schedule-sections' )    // model container
                ]
            );
        ...
        nodeNewSection.updateFacultyWeeklySchedule_AdminPageFrameworkRepeatableSections();

        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.faculty-weekly-schedule-fields.sortable' ).each( function() {
            $( this ).enableFacultyWeeklySchedule_AdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.faculty-weekly-schedule-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.faculty-weekly-schedule-fieldrow, .faculty-weekly-schedule-fieldset, .faculty-weekly-schedule-fields, .faculty-weekly-schedule-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute(
                    'id',
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.faculty-weekly-schedule-fields' ).incrementAttribute(
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.faculty-weekly-schedule-fieldset' ).incrementAttribute(
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removeFacultyWeeklySchedule_AdminPageFrameworkRepeatableSection = function() {

        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.faculty-weekly-schedule-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.faculty-weekly-schedule-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.faculty-weekly-schedule-section-tabs-contents' )
            .children( '.faculty-weekly-schedule-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.faculty-weekly-schedule-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.aFacultyWeeklySchedule_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.faculty-weekly-schedule-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.faculty-weekly-schedule-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() {
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.faculty-weekly-schedule-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.faculty-weekly-schedule-section-title' ).first().show();
                }
            }
        ...
        _oNextAllSections.each( function( _iIterationIndex ) {
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.faculty-weekly-schedule-field' ).each( function( iFieldIndex ) {
...
( function( $ ) {
  ...
  $( document ).ready( function() {
    ...
    $( '.faculty-weekly-schedule-section-tabs-contents' ).createTabs();
  });
...
(function($) {
    ...
    // Initialize
    $( document ).ready( function() {
        $( '.faculty-weekly-schedule-fields.sortable' ).each( function() {
            $( this ).enableFacultyWeeklySchedule_AdminPageFrameworkSortableFields();
        } );
    });

    $.fn.enableFacultyWeeklySchedule_AdminPageFrameworkSortableFields = function( sFieldsContainerID ) {
...
(function($) {
    ...
    // Initialization
    $( document ).ready( function() {
        $( '.faculty-weekly-schedule-sections.sortable-section' ).each( function(){
            $( this ).enableFacultyWeeklySchedule_AdminPageFrameworkSortableSections();
        } );
    });

    $.fn.enableFacultyWeeklySchedule_AdminPageFrameworkSortableSections = function( sSectionsContainerID ) {
        ...
        var _bIsTabbed      = _oTarget.hasClass( 'faculty-weekly-schedule-section-tabs-contents' );
        var _bCollapsible   = 0 < _oTarget.children( '.faculty-weekly-schedule-section.is_subsection_collapsible' ).length;

        _oTarget        = _bIsTabbed
            ? _oTarget.find( 'ul.faculty-weekly-schedule-section-tabs' )
            : _oTarget;
        ...
        var _aSortableOptions = {
                items: _bIsTabbed
                    ? '> li:not( .disabled )'
                    : '.faculty-weekly-schedule-section:not( .disabled, .faculty-weekly-schedule-collapsible-toggle-all-button-container )',
                handle: _bCollapsible
                    ? '.faculty-weekly-schedule-section-caption'
                    : false,
                ...
                stop: function(e,ui) {
                    ...
                    // Callback the registered callback functions.
                    $( this ).trigger(
                        'faculty-weekly-schedule_stopped_sorting_sections',
                ...
                // cancel: '.faculty-weekly-schedule-section-description, .faculty-weekly-schedule-section-title'
        ...
        if ( ! _bIsTabbed ) {
            ...
            _oSortable.on( 'sortstop', function() {
                ...
                $( this ).find( 'caption > .faculty-weekly-schedule-section-title:not(.faculty-weekly-schedule-collapsible-sections-title,.faculty-weekly-schedule-collapsible-section-title)' ).first().show();
                $( this ).find( 'caption > .faculty-weekly-schedule-section-title:not(.faculty-weekly-schedule-collapsible-sections-title,.faculty-weekly-schedule-collapsible-section-title)' ).not( ':first' ).hide();
  ...
  function debugLog( ...message ) {
    if ( ! parseInt( FacultyWeeklySchedule_AdminPageFrameworkScriptFormMain.debugMode ) ) {
      return;
    }
...
(function($){
  ...
  $.widget( 'faculty-weekly-schedule.pointerTooltip', $.wp.pointer, {
   /**
 ...
 *
 * It provides the $[ 'faculty-weekly-schedule-tooltip' ]( options ) jQuery plugin method.
 * The name uses hyphens for the user's text domain to be replaced. So the compiled framework script will be entirely ported with without the keyword of `faculty-weekly-schedule`.
 *
 * To use the method,
 * ```
 * $( '.my-tooltip' )[ 'faculty-weekly-schedule-form-tooltip' ]( 'Hi, this is a tooltip content!' );
 * ```
 * Or have a text message enclosed in an element with a class `faculty-weekly-schedule-form-tooltip-content`.
 * ```
 *    <span class="my-tooltip dashicons dashicons-editor-help">
 *      <span class="faculty-weekly-schedule-form-tooltip-content">
 *        Some text
 *      </span>
 *    </span>
 * ```
 * Then, call it like
 * ```
 * $( '.my-tooltip' )[ 'faculty-weekly-schedule-form-tooltip' ]();
 * ```
 * Or
 * ```
 ...
 * ```
 *
 * If it the script is loaded, elements with the .faculty-weekly-schedule-from-tooltip class will be automatically parsed.
 * So not to call the `[ 'faculty-weekly-schedule-form-tooltip' ]()` method, just create an element with the selector
 * and it should automatically have a tooltip.
 *
 *
 * When the framework file is compiled, replace the keyword `faculty-weekly-schedule` with your text domain.
 */
(function($){

  // Initialize
  $( document ).ready( function() {
    $( '.faculty-weekly-schedule-form-tooltip' ).each( function() {
      $( this )[ 'faculty-weekly-schedule-form-tooltip' ]();
    } );
  } );

  $.fn[ 'faculty-weekly-schedule-form-tooltip' ] = function( options ) {
  ...
  function initialize( target, options ) {
    ...
    options = $.extend( {}, {
      pointerClass: 'faculty-weekly-schedule-form-tooltip-balloon',
  ...
  function handleTooltip( self, options ) {
    ...
    var _content   = 'undefined' !== typeof options.content
      ...
      : $( self ).find( '.faculty-weekly-schedule-form-tooltip-content' ).clone();
    ...
    var _options    = $.extend( true, {}, options, {
      ...
      position: {
        ...
        within: _offscreen ? _body : $( self ).closest( '.faculty-weekly-schedule-field, .faculty-weekly-schedule-fieldrow, .faculty-weekly-schedule-section' ),
      },
      buttons: function() {},
      close: function() {},
    }, options );
  ...
  function handleAutoClose( self, content ) {
      ...
      $( self ).add( '.faculty-weekly-schedule-form-tooltip-balloon' ).on( 'mouseleave', function( event ){
      ...
      $( self ).add( '.faculty-weekly-schedule-form-tooltip-balloon' ).on( 'mouseenter', function(){
    ...
    $( document ).ready( function() {
        ...
        $( document ).ajaxComplete( function( event, XMLHttpRequest, ajaxOptions ) {
            ...
            if( _aRequest.action && ( 'save-widget' === _aRequest.action ) ) {
                ...
                if ( $( _oWidget ).find( '.faculty-weekly-schedule-sectionset' ).length <= 0 ) {
                    return;
                }
                ...
                $( document ).trigger(
                    'faculty-weekly-schedule_saved_widget',
                    _oWidget
                );
