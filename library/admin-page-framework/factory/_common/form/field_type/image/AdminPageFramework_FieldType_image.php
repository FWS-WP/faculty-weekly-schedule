...
class FacultyWeeklySchedule_AdminPageFramework_FieldType_image extends FacultyWeeklySchedule_AdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'faculty-weekly-schedule-field-type-image', 'src' => dirname(__FILE__) . '/js/image.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'faculty-weekly-schedule-script-form-main' ), 'translation_var' => 'FacultyWeeklySchedule_AdminPageFrameworkImageFieldType', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'referer' => 'admin_page_framework', 'hasMediaUploader' => function_exists('wp_enqueue_media'), 'label' => array( 'uploadImage' => $this->oMsg->get('upload_image'), 'useThisImage' => $this->oMsg->get('use_this_image'), 'insertFromURL' => $this->oMsg->get('insert_from_url'), ), ), ), );
    }
...
jQuery(document).ready(function(){setFacultyWeeklySchedule_AdminPageFrameworkImageUploader('{$sInputID}','true'==='{$_sRepeatable}','true'==='{$_bExternalSource}')})
JAVASCRIPTS;
        return "<script type='text/javascript' class='faculty-weekly-schedule-image-uploader-button'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>". PHP_EOL;
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='faculty-weekly-schedule-{$sType}-remove-button'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>". PHP_EOL;
    }
