...
abstract class FacultyWeeklySchedule_AdminPageFramework_PostType extends FacultyWeeklySchedule_AdminPageFramework_PostType_Controller {
    protected $_sStructureType = 'post_type';
    public function __construct($sPostType, $aArguments=array(), $sCallerPath=null, $sTextDomain='faculty-weekly-schedule')
    {
        if (empty($sPostType)) {
            return;
        }
        ...
        $this->oProp->sPostType = FacultyWeeklySchedule_AdminPageFramework_WPUtility::sanitizeSlug($sPostType);
    ...
    private function _getCallerScriptPath($sCallerPath)
    {
        ...
        $_sPageNow = FacultyWeeklySchedule_AdminPageFramework_Utility::getElement($GLOBALS, 'pagenow');
        if (in_array($_sPageNow, array( 'edit.php', 'post.php', 'post-new.php', 'plugins.php', 'tags.php', 'edit-tags.php', 'term.php' ))) {
            return FacultyWeeklySchedule_AdminPageFramework_Utility::getCallerScriptPath(__FILE__);
        }
        return null;
    }
}
